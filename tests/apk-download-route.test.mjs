...
test("the public Android download route returns the complete APK as an attachment", async () => {
  const apk = await readFile(new URL("../public/downloads/daxiang-abao-alarm.apk", import.meta.url));
  const response = await worker.fetch(
    new Request("https://example.com/download/android"),
    {
      ASSETS: {
        fetch: async (request) => new URL(request.url).pathname === "/downloads/daxiang-abao-alarm.apk"
  ...
  assert.match(response.headers.get("content-disposition") ?? "", /^attachment; filename="daxiang-abao-alarm\.apk"$/);
