...
import { consents, conversations, goalCheckins, goals, healthSubjects, users } from "../../../db/schema";
...
export async function GET(request: Request) {
  ...
  const [permissionRows, conversationRows, goalRows, checkinRows] = await Promise.all([
    ...
    database.select({ goalId: goalCheckins.goalId, createdAt: goalCheckins.createdAt }).from(goalCheckins).where(eq(goalCheckins.ownerUserId, identity.userId)).orderBy(desc(goalCheckins.createdAt)).limit(200),
  ]);
  return Response.json({ user: { id: identity.userId, email: identity.email, displayName: identity.displayName }, subject, consents: permissionRows, conversations: conversationRows, goals: goalRows, checkins: checkinRows });
}
