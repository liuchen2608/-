...
只返回 JSON，不用 Markdown 围栏，格式 {"reply":"简体中文回复，最多 800 字","responseType":"greeting 或 clarification 或 recommendation 或 boundary_refusal 或 safety_stop","category":"饮食 或 作息 或 运动 或 习惯 或 一天计划 或 打个招呼 或 补充说明 或 边界说明 或 安全中止"}。
不输出思考过程、链接或 HTML。clarification 用自然追问而非拒绝；推荐末尾加“以上仅为日常生活方式参考”。`;
...
function parseReply(raw: string): GuardResult {
  const value = JSON.parse(raw);
  if (!value || Array.isArray(value) || typeof value.reply !== "string") throw new Error("invalid_response");
  if (value.responseType === "boundary_refusal") return { reply: BOUNDARY_REPLY, responseType: "boundary_refusal", category: "边界说明" };
  if (value.responseType === "safety_stop") return { reply: SAFETY_STOP_REPLY, responseType: "safety_stop", category: "安全中止" };
  ...
  if (value.responseType === "greeting") return { reply, responseType: "greeting", category: "打个招呼" };
  if (value.responseType === "clarification") return { reply, responseType: "clarification", category: "补充说明" };
  if (value.responseType !== "recommendation" || !["饮食", "作息", "运动", "习惯", "一天计划"].includes(value.category)) throw new Error("invalid_response");
  return { reply: reply.includes("以上仅为日常生活方式参考") ? reply : `${reply}\n\n以上仅为日常生活方式参考。`, responseType: "recommendation", category: value.category };
}
...
export async function answerDialogue(input: {
  ...
  const fallback = (status: DialogueAnswer["status"], error?: string): DialogueAnswer => ({ ...guard, provider: "rules_fallback", modelVersion: "lifestyle-guard-v3", status, ...(error ? { error } : {}) });
  ...
  try {
    const response = await (dependencies.fetch ?? fetch)("https://api.deepseek.com/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          ...selectDialogueHistory(input.history),
    ...
    const answer = parseReply(choice.message.content);
