...
用户明确要求在现有对话框接入 DeepSeek 需求对话，并确认在使用者授权后，将本次输入、当前对话最近最多 12 条消息和主动保存的生活偏好发送给 DeepSeek。此决定替代此前“仅为固定行动排序、不发送原文”的限制；保留非医疗范围及其他页面功能。
...
flowchart TD
  ...
  C -->|未命中| D[读取新版使用者授权]
  D -->|未授权或已撤回| F[标记为本地回复]
  D -->|已授权| E[当前消息 + 当前会话近期内容 + 筛选后的生活偏好]
  E --> M[单次 DeepSeek 请求]
  M --> G[JSON、完成状态、长度及内容检查]
  G -->|不通过或调用失败| F
  G -->|通过| H[DeepSeek 回复或固定边界语]
  L --> S[原子保存问答、摘要与审计]
  F --> S
  H --> S
```
...
继续使用现有 Vinext、React、D1、R2 和 DeepSeek，无新框架或数据库迁移。请求使用已配置模型（默认 deepseek-v4-pro）、JSON 输出、非思考模式、最多 1200 输出 token、18 秒超时，不自动重试。

严格校验完整 JSON、响应完成状态、回复长度及允许的类型/类别。医疗与紧急类型始终替换成服务端固定语；普通文本还经过禁止内容及链接/HTML 检查。React 作为纯文本展示，不执行模型 HTML 或代码。
