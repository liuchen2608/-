...
import { consents, conversations, deviceConnections, feedback, goalCheckins, goals, healthRecords, healthSubjects, messages, serviceEvents, uploads, users } from "../../../db/schema";
...
export async function POST(request:Request){
  ...
  if(action==="update_subject"){const subjectId=String(body.subjectId);const row=(await database.select().from(healthSubjects).where(and(eq(healthSubjects.id,subjectId),eq(healthSubjects.ownerUserId,identity.userId),eq(healthSubjects.authorizationStatus,"active"))).limit(1))[0];if(!row)return Response.json({error:"subject_not_found"},{status:404});const name=String(body.name||row.name).trim().slice(0,40);if(!name)return Response.json({error:"name_required"},{status:400});const profileText=String(body.profileText||"").trim().slice(0,4000);await database.update(healthSubjects).set({name,profileJson:json({profileText}),updatedAt:now()}).where(eq(healthSubjects.id,subjectId));await audit(identity.userId,"subject_profile_updated","health_subject",subjectId,subjectId,{nameChanged:name!==row.name,profileTextLength:profileText.length});return Response.json({id:subjectId,name,updated:true});}
  if(action==="delete_subject"){const subjectId=String(body.subjectId);const row=(await database.select().from(healthSubjects).where(and(eq(healthSubjects.id,subjectId),eq(healthSubjects.ownerUserId,identity.userId),eq(healthSubjects.authorizationStatus,"active"))).limit(1))[0];if(!row)return Response.json({error:"subject_not_found"},{status:404});if(row.relation==="self")return Response.json({error:"self_subject_requires_account_deletion"},{status:409});const storedFiles=await database.select({objectKey:uploads.objectKey}).from(uploads).where(and(eq(uploads.ownerUserId,identity.userId),eq(uploads.subjectId,subjectId)));if(storedFiles.length)await Promise.all(storedFiles.map(file=>files().delete(file.objectKey)));await database.delete(healthSubjects).where(eq(healthSubjects.id,subjectId));await audit(identity.userId,"subject_deleted","health_subject",subjectId,null,{relation:row.relation,deletedFileCount:storedFiles.length,cascade:true});return Response.json({id:subjectId,deleted:true});}
