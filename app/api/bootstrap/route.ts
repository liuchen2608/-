...
import { consents, conversations, deviceConnections, goals, healthRecords, healthSubjects, uploads, users } from "../../../db/schema";
...
export async function GET(request:Request){
  ...
  const [permissionRows,conversationRows,recordRows,uploadRows,goalRows,deviceRows]=await Promise.all([
    ...
    database.select({id:uploads.id,subjectId:uploads.subjectId,fileName:uploads.fileName,contentType:uploads.contentType,size:uploads.size,status:uploads.status,createdAt:uploads.createdAt}).from(uploads).where(eq(uploads.ownerUserId,identity.userId)).orderBy(desc(uploads.createdAt)).limit(50),
  ...
  return Response.json({user:{id:identity.userId,email:identity.email,displayName:identity.displayName},subjects,consents:permissionRows,conversations:conversationRows,records:recordRows,uploads:uploadRows,goals:goalRows,devices:deviceRows});
}
